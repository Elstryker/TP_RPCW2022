...
router.get("/", function (req, res) {
  if (!req.cookies.token) aux.gerarTokenConsumidor(req.originalUrl, res);
    ...
    axios
      .get("http://localhost:10000/api/recursos?token=" + req.cookies.token)
      .then((dados) => {
...
router.post("/file", upload.single("file"), async function (req, res) {
  if (!req.cookies.token) auxFiles.gerarTokenConsumidor(req.originalUrl, res);
    ...
    let newPath = pathLib.join(
      __dirname,
      "/../uploads/",
      req.file.originalname + ".zip"
    );
    ...
    files
      .then(function (files) {
        ...
        for (var i = 0; i < files.length; i++) {
          var file = files[i];
          var fileActualPath = pathLib.normalize(
            __dirname.replace("/routes", "/public/files" + file)
          );
          var fileObj = {
        ...
        var dataAtual = new Date().toISOString().substr(0, 19);
            ...
            axios.post('http://localhost:10000/api/recursos?token=' + req.cookies.token, {recurso: recursoObj})
              ...
              axios
                .post(
                  "http://localhost:10000/api/noticias?token=" +
                    req.cookies.token,
                  { noticia: noticiaObj }
                )
                .then((dados) => res.redirect("/perfil"))
...
router.post("/download", (req, res) => {
  if (!req.cookies.token) aux.gerarTokenConsumidor(req.originalUrl, res);
    ...
    axios
      .get(
        "http://localhost:10000/recursos/:id" +
          req.params.id +
          "token=" +
          req.cookies.token
      )
      .then((recursoData) => {
        ...
        axios
          .post(
            "http://localhost:10000/recursos/download?token=" +
              req.cookies.token,
            req.params.id
          )
          .then(() => {
